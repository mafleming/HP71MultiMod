...
;      version 5.45 which uses a different assembler from the old MPASM.       *
...
#ifdef  XTRNBOOT
; Add MCC generated C bootloader project to Configuration/Loading
...
IDLELP:
        ...
        bra     NOBREAK             ; No
        btfss   BANKMASK(RC1STA),RC1STA_FERR_POSN,b       ; Framing Error?
        bra     NOBREAK             ; No
        banksel CMD
        btfsc   PORTC,7,c           ; Receive line low?
        bra     NOBREAK             ; No
...
NOBREAK:
        ...
        banksel PIR3
        btfsc   RC1IF               ; Receive Interrupt bit set?
...
; Taken from mcc_generated_files/pin_manager.c
        ...
        movwf   TRISC,b
        banksel ANSELA
        movlw   0xA0                ; Disable unused digital inputs
        movwf   ANSELA,b
        movlw   0xF0
        movwf   ANSELB,b
        movlw   0x00
...
;//<editor-fold defaultstate="open" desc="Serial Monitor">
        ...
        banksel PMD4
        bcf     UART1MD              ; Enable EUSART1
        banksel TRISC
        bcf     TRISC,6,b            ; EUSART1 RC6 is TX, enable output
        ;movlw   0xbf                ; EUSART1 RC6 is TX
        ;movwf   TRISC,1
        bsf     TRISC,7,b            ; EUSART1 RC7 is RX, disable output
        bcf     ANSELC,7,b           ; EUSART1 RC7 is RX, enable digital input
        bsf     WPUC,7,b             ; Add weak pullup if serial not connected
...
; From eusart1.c
        ...
        movwf   BAUD1CON,b          ; enabled, autobaud disabled
